...
# -----------------------------------------------------
# Datentypen (ÄNDERUNG 2: Ergänzt zur Behebung der FutureWarning)
# -----------------------------------------------------
for col in [
    "league_group",
    "category",
...
# =====================================================
# Regeln (ÄNDERUNG 1: Eine einzige, zusammenhängende Liste)
# =====================================================
...
for i, row in df.iterrows():
    ...
    if not found:
        youth = [
            ...
            "Youth", "Nachwuchsliga", "Jugendliga", "Drenge",
            "Junior", "Jgd", "YFL",
        ]

        if any(x in league for x in youth):
            row["league_group"] = "Youth"
            row["category"] = "Youth"
            row["level"] = None
            row["is_youth"] = True
            found = True
    ...
    if not found:
        reserve = [
            "Reserve", "Reserves", "II", "Primavera",
            "Premier League 2", "Next Pro", "NextGen",
        ]

        if any(x in league for x in reserve):
            row["league_group"] = "Reserve"
...
print("Kategorien:")
print(df["category"].fillna("Unbekannt").value_counts())
print()

# ÄNDERUNG 3: Fängt leere Strings "" zusätzlich ab
print("Fehlende Gruppen:")
print(((df["league_group"].fillna("").eq(""))).sum())
print()

print("Professional:")
print(df["is_professional"].value_counts())
print()

print("Youth:")
print(df["is_youth"].value_counts())
print()

print("Reserve:")
print(df["is_reserve"].value_counts())
print()
